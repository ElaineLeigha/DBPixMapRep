...
//    scene->addPixmap(pix);
    ...
    powertimer->stop();
    connect(powertimer, SIGNAL(timeout()), this, SLOT(starttimer()));
...
void MainWindow::starttimer()
{
    ...
    //****************************Show powered off sections********************
        ...
        while(two.next())
        {
             QString section = two.value(0).toString();
             bool power = two.value(1).toInt();
...
void MainWindow::sendinfotrack()
{
    ...
    while(q.next())
    {
        DS.append(q.value(0).toString());  //assign the name of the detection section to an array variable
        if(ui->powerBox->findText(DS.at(i)) == -1)
        {
            if(ui->powerBox->currentText() != DS.at(i) || ui->powerBox->count() ==0)
            {
                ui->powerBox->addItem(DS.at(i));
            }
        }
...
void MainWindow::deactivate(QString section,bool power)
{
    ...
    {
        if(DS.at(i) == section && power == false)
        {
            DSections[i]->setOpacity(0.2);
        }
        else if(DS.at(i) == section && power == true)
        {
